# Adapted and extended from https://github.com/RaRe-Technologies/gensim/blob/develop/gensim/corpora/wikicorpus.py
 
# Code  Issues 231  Pull requests 26  Projects 1  Wiki  Security  Pulse  Community
# gensim/gensim/corpora/wikicorpus.py
# @horpto horpto Remove unnecessary creations of lists at all (#2261)
# c3d2299 on 14 Dec 2018
# @piskvorky @josephcc @steremma @menshikh-iv @larsmans @horpto @lukaselmer @zsef123 @roopalgarg @polm @mrshu @macks22 @mattilyra @tmylk @kirit93 @jbochi @ziky90 @isomap @franklsf95 @cscorley @chaitaliSaini @kataev @anotherbugmaster @alekol
# 717 lines (591 sloc)  25.5 KB
...
RE_P18 = re.compile(r'===(.*?)===',re.UNICODE)
RE_P19 = re.compile(r'==(.*?)==',re.UNICODE)
RE_P20 = re.compile(r'\'\'',re.UNICODE)
RE_P21 = re.compile(r'\'',re.UNICODE)
RE_P22 = re.compile(r'\n',re.UNICODE)
...
def filter_wiki(raw, promote_remaining=True, simplify_links=True, remove_newline = True):
    ...
    return remove_markup(text, promote_remaining, simplify_links, remove_newline)


def remove_markup(text, promote_remaining=True, simplify_links=True, remove_newline=True):
...
#         text = text[:text.index('== References ==')]
        ...
        # remove table markup
        text = text.replace("!!", " ")  # each table head cell on a separate line
        text = text.replace("|-||", " ")  # for cases where a cell is filled with '-'
        text = re.sub(RE_P12, ' ', text)  # remove formatting lines
        text = text.replace('|||', ' ')  # each table cell on a separate line(where |{{a|b}}||cell-content)
        text = text.replace('||', ' ')  # each table cell on a separate line
        text = re.sub(RE_P13, ' ', text)  # leave only cell content
        text = re.sub(RE_P17, ' ', text)  # remove formatting lines
        text = re.sub(RE_P18, ' ', text) # remove headers, e.g. == Biography ==
        text = re.sub(RE_P19, ' ', text) # remove subheaders
        text = re.sub(RE_P20, '"', text) # remove formatting of quotes
        text = re.sub(RE_P21, ' ', text) # remove formatting of apostrophes
        if remove_newline:
            text = re.sub(RE_P22, ' ', text) # remove \n
